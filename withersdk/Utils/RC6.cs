...
namespace withersdk.Utils
{
    public class RC6
    {
        ...
        private uint[] RoundKey = new uint[2 * R + 4];  // ключ раунда
        private const int W = 32; // длина машинного слова в битах
        private byte[] MainKey; // ключ
        ...
        private uint RightShift(uint value, int shift)
        {
        ...
        private void GenerateKey(int Long, byte[] keyCheck)
        {
            ...
            A = B = 0;
            i = j = 0;
            int V = 3 * System.Math.Max(c, 2 * R + 4);  // максимум из раундов или количества слов в ключе
            for (int s = 1; s <= V; s++)
            {
        ...
        private byte[] ToArrayBytes(uint[] uints, int Long)
        {
        ...
        public static byte[] Encode(byte[] plaintext, int Long, byte[] keyCheck = null)
        {
            var rc6 = new RC6(Long, keyCheck);
            var enc = rc6.Encode(plaintext);
            rc6 = null;
            return enc;
        }
        public static byte[] Decode(byte[] ciphertext, int Long, byte[] keyCheck = null)
        {
            var rc6 = new RC6(Long, keyCheck);
            var dec = rc6.Decode(ciphertext);
            rc6 = null;
            return dec;
        }
        public byte[] Encode(byte[] plaintext)
        {
            ...
            for (i = 0; i < text.Length; i = i + 16)
            {
                ...
                for (int j = 1; j <= R; j++)
                {
                    uint t = LeftShift((B * (2 * B + 1)), (int)(System.Math.Log(W, 2)));
                    uint u = LeftShift((D * (2 * D + 1)), (int)(System.Math.Log(W, 2)));
        ...
        public byte[] Decode(byte[] cipherText)
        {
            ...
            return plainText.Where((item, index) => item != 0).ToArray();
        }
    }
}
